...
"""
Server script for running the Morgana API.
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run the Morgana API server")
    parser.add_argument(
        "--reload",
        action="store_true",
    ...
    try:
        logger.info(f"Starting Morgana API server on {args.host}:{args.port}")
        uvicorn.run(
            "src.server:app",
            host=args.host,
            port=args.port,
