...
async def _execute_deepagent_step(
    ...
    # Get all plan steps
    all_steps = current_plan.steps if current_plan.steps else []
    
    if not all_steps:
        logger.warning("No steps in plan to execute.")
    ...
    # Store all execution results for final synthesis
    all_execution_results = []
    
    # Loop through steps in pairs (2 at a time)
    for i in range(0, len(all_steps), 2):
        # Take 2 steps at a time (or just 1 if it's the last remaining step)
        steps_to_execute = all_steps[i:i+2]
        
        logger.info(f"Executing pair {i//2 + 1}: {len(steps_to_execute)} step(s) - {[s.title for s in steps_to_execute]}")
        
        # Build a summary of the current pair of steps
        steps_summary = ""
        for idx, step in enumerate(steps_to_execute, start=1):
            steps_summary += f"## Step {i + idx}: {step.title}\n\n"
            steps_summary += f"**Description:** {step.description}\n\n"
            steps_summary += f"**Step type:** {step.step_type}\n\n"
            steps_summary += f"**Requires search:** {step.need_search}\n\n"
        ...
        logger.info("Executing deep_agent for pair %d (%d step(s)).", i//2 + 1, len(steps_to_execute))
        logger.debug("Deep agent input messages: %s", primary_message_content)
        
        # Execute the agent for this pair of steps
        result = await agent.ainvoke(input={"messages": messages})
        
        # Extract the response from this execution
        step_response_content, _ = _extract_final_report_from_result(result)
        step_response_content = step_response_content.strip()
        
        logger.debug("Pair %d response: %s", i//2 + 1, step_response_content[:500])
        
        # Store the result
        all_execution_results.append({
            "pair_number": i//2 + 1,
            "steps": steps_to_execute,
            "response": step_response_content,
            "messages": result.get("messages", [])
        })
        
        # Mark these steps as having intermediate execution results
        for step in steps_to_execute:
            step.execution_res = step_response_content or f"Completed in pair {i//2 + 1}"
    
    # NOW: All pairs have been executed. Time to synthesize everything.
    logger.info("All %d pairs executed. Now synthesizing final comprehensive report...", len(all_execution_results))
    
    try:
        synthesizer_llm = get_llm_by_type(AGENT_LLM_MAP["reporter"])
        
        # Gather ALL messages from ALL executions
        all_messages_from_all_runs: list[BaseMessage] = []
        for exec_result in all_execution_results:
            all_messages_from_all_runs.extend(exec_result["messages"])
        
        original_human_query = user_brief
        plan_summary = _render_plan_summary(current_plan)

        # Load synthesizer prompt
        try:
            synthesizer_system_prompt = apply_prompt_template(
                "synthesizer", state, configurable
            )[0]
            if not isinstance(synthesizer_system_prompt, SystemMessage):
                raise ValueError("Loaded prompt was not a SystemMessage.")
            logger.info("Successfully loaded and rendered synthesizer.md prompt.")
        except Exception as e:
            logger.error(
                f"Failed to load synthesizer.md template: {e}. Using fallback prompt."
            )
            synthesizer_system_prompt = SystemMessage(
                content=dedent(
                    """
                    You are an expert report synthesizer. Your task is to transform ALL research data from multiple research iterations into ONE comprehensive, detailed final report (3,000-10,000 words).
                    
                    **Instructions:**
                    1. **Synthesize, Don't Summarize:** Create a NEW comprehensive report by integrating ALL information from ALL research iterations.
                    2. **Use ALL Data:** Extract findings, data, and citations from ALL tool outputs and research iterations.
                    3. **Address the Plan:** Ensure the report covers ALL steps from the research plan.
                    4. **Answer the Query:** Directly address the original user question comprehensively.
                    5. **Rich Formatting:** Use tables, lists, sections, and markdown formatting extensively.
                    6. **Comprehensive Length:** Aim for 3,000-10,000 words of detailed content.
                    7. **Cite Sources:** Include all citations in a "Key Citations" section at the end.
                    8. **Professional Quality:** Make it publication-ready with clear structure and flow.
                    
                    **Output:**
                    Produce ONLY the final comprehensive markdown report.
                    """
                )
            )
        # Build the comprehensive context for synthesis
        execution_summary = "# All Research Execution Results\n\n"
        for exec_result in all_execution_results:
            execution_summary += f"## Pair {exec_result['pair_number']}\n\n"
            execution_summary += f"**Steps Covered:**\n"
            for step in exec_result['steps']:
                execution_summary += f"- {step.title}: {step.description}\n"
            execution_summary += f"\n**Research Findings:**\n{exec_result['response']}\n\n"
            execution_summary += "---\n\n"

        synthesizer_prompt_messages = [
            synthesizer_system_prompt,
            HumanMessage(
                content=dedent(
                    f"""
                    Here is ALL the context for synthesizing the final comprehensive report:
                    ...
                    ---
                    ## 3. Summary of All Research Executions
                    {execution_summary}

                    ---
                    ## 4. Complete Message History from All Research Iterations
                    Below is the FULL message log from ALL research agent executions. This is your PRIMARY source material.
                    """
                )
            ),
            *all_messages_from_all_runs,
            HumanMessage(
                content=dedent(
                    """
                    ---
                    ## 5. Your Task
                    Based on ALL the information provided above (User Query, Complete Plan, All Research Findings, and Full Message History), 
                    generate a COMPREHENSIVE, DETAILED final report (3,000-10,000 words) that:
                    - Addresses EVERY point in the user's question
                    - Covers EVERY step in the research plan
                    - Uses tables, sections, and rich formatting
                    - Includes all relevant data and citations
                    - Is publication-ready quality
        ...
        logger.info(
            f"Invoking synthesizer LLM with {len(synthesizer_prompt_messages)} messages."
        )
        synthesis_response = await synthesizer_llm.ainvoke(synthesizer_prompt_messages)
        response_content = synthesis_response.content.strip()
        # We *created* the definitive final report
        has_final_report_payload = True

    except Exception as e:
        logger.exception(
            f"CRITICAL: Extensive report synthesis failed: {e}. Falling back to basic extraction."
        )

        # Fallback: concatenate all execution results
        response_content = "\n\n---\n\n".join([er["response"] for er in all_execution_results])
        has_final_report_payload = False
        
    # --- [END] NEW EXTENSIVE REPORT SYNTHESIS STEP ---
    
    
    logger.info("Final comprehensive report generated (%d chars).", len(response_content))
    logger.debug("Final report preview: %s", response_content[:500])
